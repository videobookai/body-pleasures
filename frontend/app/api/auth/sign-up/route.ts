...
const callStrapi = async (path: string, body: Record<string, unknown>) => {
  ...
  if (!resp.ok) {
    const message = json?.error?.message || json?.message || resp.statusText || "Registration failed.";
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!body?.username || !body?.email || !body?.password) {
      return sendError("Username, email, and password are required.");
    }

    const payload = await callStrapi("auth/local/register", {
